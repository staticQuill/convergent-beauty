...
export const userAuthStore = defineStore({
  ...
  actions: {
    async login(username: string, password: string) {
      ...
      const tokensJson = await fetch("https://188.166.174.54:8080/auth/login/", authRequestOptions);
    ...
    async signup(username: string, email: string, password: string, password2: string) {
      ...
      const signupJson = await fetch("https://188.166.174.54:8080/auth/signup/", authRequestOptions);
