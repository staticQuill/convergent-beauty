...
export const userAuthStore = defineStore({
  ...
  actions: {
    async login(username: string, password: string) {
      ...
      const tokensJson = await fetch("https://convergent.beauty/api/auth/login/", authRequestOptions);
    ...
    async signup(username: string, email: string, password: string, password2: string) {
      ...
      const signupJson = await fetch("https://convergent.beauty/api/auth/signup/", authRequestOptions);
